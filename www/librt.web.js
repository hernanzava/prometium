...
dbgUi = function (quiereOn) {
    if (quiereOn) {
        if (enAppMovil) {
            CFGLIB.loglvlmax = 5;
            alertNative = alertNative || alert;
            alert = function (m) {
                m.match(/:ERR:/) ? xalert(m) : showMsg(m);
            }
        }
    } else {
        if (enAppMovil) {
...
function showMsg(msg) {
    alert(msg);
} //XXX: Cambiar por una función con el estilo de la UI
function raiseError(msg) {
    showMsg(msg);
} //XXX: Cambiar por una función que guarde en el log y muestre un cartel (¿Usar Log de lib.js?)
function fnop() {}; //D: NOP

toJs = function (v) {
        return v;
    } //D: compatibilidad con rt_java
...
function libRequire(file, urlBase, url, cb) { //D: load js modules, call BEFORE using, asume async loading -> call dependent code from libStart that is always called AFTER loading all previously required modules
    ...
    function processData(response) {
        ...
        if (needsEval) {
            try {
                ...
                alert("[LRQ] Error evaluando file='" + file + "' " + ex.message + " " + ex.fileName + " " + ex.lineNumber);
            }
        }
        if (cb) {
            cb(d, raw, url);
    ...
    if (Cache.LibEval[url]) {
        ...
        if (enAppMovil) { //A: mobile, para que funcione offline Y online
            ...
            var s0 = function () {
                DBG.run && alert("DBG:libRequire getHttpOrDflt file='" + file + "' url='" + url + "'");

                getHttpToDflt(file, url, s1, s1);
            }
            
            var s1 = function () {
                DBG.run && alert("DBG:libRequire evalFileOrDflt file='" + file + "' url='" + url + "'");
    ...
    if (Object.keys(LibWaiting).length == 0) { //A: no hay pendientes para cargar
        ...
        if (cb) {
            try {
                ...
                alert("[RS] ERROR iniciando " + ex.message + " " + ex.stack);
            }
        }
...
ser_json = function (o, wantsPretty) {
    ...
    if (o != null) {
        try {
            s = JSON.stringify(o, null, wantsPretty ? 2 : null);
        } catch (ex) {
            s = o + "";
        }
    } else {
...
ser_planoOjson = function (o, wantsPretty) {
    return ser_planoOproto(o, ser_json, wantsPretty);
}
ser_planoOjson_r = function (s) {
    return ser_planoOproto_r(s, ser_json_r);
}


load = function (l) {
    console.log("REVISAR QUE " + l + " ESTE CARGADO")
};
...
cxAjax = function (cmd, args, pipe, cb) {
    ...
    $.ajax({
        ...
        error: function (xhr, status, error) {
            showMsg('Ajax error!' + status + " " + xhr.responseText + " " + JSON.stringify(req))
        }
    });
...
funcionConCache_a_archivos = function (nombre, funcion, cbIdx, funcionClavePara, cacheCntMax, prefijoNombreArchivo) { //D: envuelve una funcion con cache, si esta en movil usa archivos encriptados para cuando este offline
    ...
    if (!enAppMovil && CACHE_LOCALSTORAGE) {
        ...
        getFile = function (p, t, cbok, cbf) {
            var v = localStorage[p];
            setTimeout(function () {
                v ? cbok(v) : cbf()
            }, 100);
        }
        setFile = function (p, v, cbok, cbf) {
            localStorage[p] = v;
            setTimeout(cbok, 100);
        }
        setFileBin = function (p, v, cbok, cbf) {
            localStorage[p] = v;
            setTimeout(cbok, 100);
        }
        setFileDir = function (p, cbok, cbf) {
            setTimeout(cbok, 100);
        }
        borrarTodo_dir = function (p, quiereSinPedirConfirmacion, cb) {
            for (var i = 0; i < localStorage.length; i++) {
                var k = localStorage.key(i);
                if (k.substr(0, p.length) == p) {
...
if (document.body) {
    document.body.innerHTML = "CARGANDO de " + appHost + "...";
} //XXX: generalzar, pantalla de inicio